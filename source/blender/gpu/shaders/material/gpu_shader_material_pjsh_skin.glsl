...
void node_bsdf_pjsh_skin(vec4 diffuse,
                         vec4 specular,
                         float roughness,
                         vec4 emissive,
                         float transp,
...
//
//  ClosureInputCommon cl_common = CLOSURE_INPUT_COMMON_DEFAULT;
//  for (int i = 0; i < laNumLight && i < MAX_LIGHT; i++) {
//    ClosureLightData light = closure_light_eval_init(cl_common, i);
//    //result.radiance = vec3(light.vis);
//  }
  //ClosureInputCommon cl_common = CLOSURE_INPUT_COMMON_DEFAULT;
  result = ambient_occlusion_eval(N, 1.0, 0.0, 16.0) * laNumLight;
}
