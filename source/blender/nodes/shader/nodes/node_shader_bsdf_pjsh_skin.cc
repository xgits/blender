...
namespace blender::nodes::node_shader_bsdf_pjsh_skin_cc {
...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_output<decl::Float>(N_("BSDF"));
}

#define socket_not_zero(sock) (in[sock].link || (clamp_f(in[sock].vec[0], 0.0f, 1.0f) > 1e-5f))

static int node_shader_gpu_bsdf_pjsh_skin(GPUMaterial *mat,
                                          bNode *node,
                                          ...
                                          GPUNodeStack *out)
{
  ...
  return GPU_stack_link(mat, node, "node_bsdf_pjsh_skin", in, out, GPU_constant(&use_clear));
}

}  // namespace blender::nodes::node_shader_eevee_specular_cc

/* node type definition */
void register_node_type_sh_bsdf_pjsh_skin()
{
  namespace file_ns = blender::nodes::node_shader_bsdf_pjsh_skin_cc;

  static bNodeType ntype;

  sh_node_type_base(&ntype, SH_NODE_BSDF_PJSH_SKIN, "PJSH Skin BSDF", NODE_CLASS_INPUT);
  ntype.declare = file_ns::node_declare;
  node_type_gpu(&ntype, file_ns::node_shader_gpu_bsdf_pjsh_skin);
